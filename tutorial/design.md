...
Main Container
├─ Initial Loading (全屏初始加载提示)
├─ Loading Progress Bar (顶部固定进度条)
├─ Sidebar (左侧边栏/移动端侧滑)
│  └─ 子菜单（可展开/收缩）
│     └─ 文档标题导航（二级子菜单）
└─ Content Area (内容区)
...
```
tutorial/
├── index.html              # 主页面（包含初始加载提示和mermaid预加载）
├── styles.css              # 样式文件（包含初始加载提示样式）
├── js/                     # JavaScript模块目录
│   ├── core.js            # 核心配置和状态管理
│   ├── cache.js           # 缓存管理模块
│   ├── menu.js           # 菜单处理模块
│   ├── content.js        # 内容处理模块（改进的mermaid加载逻辑）
│   ├── utils.js          # 工具函数模块
│   └── main.js           # 应用入口模块（初始加载提示控制）
├── libs/                   # 第三方库目录
│   ├── marked.min.js
│   ├── katex.min.js
│   ├── katex.min.css
...
- **本地库文件**：
  ...
  - `libs/mermaid.min.js` - 图表渲染（HTML预加载 + 动态加载）
- **优势**：
  - 减少外部依赖，提高加载稳定性
  - 国内访问速度快，无需翻墙
  - 避免CDN失效导致页面功能异常
...
#### 10.3.3 双重加载策略
- **Mermaid图表**：HTML预加载 + 动态加载双重保障机制
- **预加载机制**：在HTML头部直接引入mermaid.min.js
- **动态加载机制**：作为备选方案，确保兼容性
- **错误处理**：渲染失败时显示原始代码（优雅降级）
